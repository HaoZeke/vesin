...
// Access VerletState for did_rebuild_flag (vesin_torch links vesin_objects)
#include "verlet.hpp"
...
VerletNeighborListHolder::VerletNeighborListHolder(
    ...
    if (skin <= 0) {
        throw std::runtime_error("skin must be > 0 for Verlet neighbor list");
    }
    data_ = new VesinNeighborList();
}
...
bool VerletNeighborListHolder::did_rebuild() {
    ...
    if (!data_->verlet_mode || data_->opaque == nullptr) {
        return false;
    }
    auto* state = static_cast<vesin::VerletState*>(data_->opaque);
    return state->did_rebuild_flag;
}
