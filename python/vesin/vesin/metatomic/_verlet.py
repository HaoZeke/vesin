...
class VerletNeighborList:
    ...
    def __init__(
        ...
        # CPU path: ctypes-based with skin > 0 in VesinOptions
    ...
    def did_rebuild(self) -> bool:
        ...
        # For CPU path, the VesinNeighborList has verlet_mode set when skin > 0.
        # We cannot easily read did_rebuild_flag from ctypes without the internal
        # header, so we track it indirectly: the first call always rebuilds,
        # and subsequent calls rebuild when displacement exceeds skin/2.
        # The C library handles this internally; we expose it through the
        # torch path when available.
        return False
    ...
    def _compute_cpu(self, system: System, device: torch.device) -> TensorBlock:
        """CPU path: ctypes-based vesin_neighbors with skin > 0."""
        self._last_was_gpu = False
        ...
        options.cutoff = self._cutoff
        options.full = self.options.full_list
