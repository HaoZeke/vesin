...
void run_gpu_verlet_test(int device_id) {
    ...
    // First call: should rebuild (fresh state)
    auto status = vesin_neighbors(
    ...
    // Second call with same positions: should reuse (no rebuild)
    auto status2 = vesin_neighbors(
        d_points, n_points, d_box, d_periodic,
        {VesinCUDA, device_id},
        options, &neighbors, &error_message
    );
    ...
    // Perturb points slightly (within skin/2 = 0.5): should still reuse
    ...
    // Large perturbation (beyond skin/2 = 0.5): should trigger rebuild
    double far_points[][3] = {
        {0.0, 0.0, 0.0},
        {1.0, 1.0, 1.0},
        {1.5, 1.5, 1.5},  // moved from (2,2,2) to (1.5,1.5,1.5)
    };
    check_cuda(cudaMemcpy(d_points, far_points, sizeof(double) * n_points * 3, cudaMemcpyHostToDevice));

    auto status4 = vesin_neighbors(
        d_points, n_points, d_box, d_periodic,
        {VesinCUDA, device_id},
        options, &neighbors, &error_message
    );
    REQUIRE(status4 == EXIT_SUCCESS);
    // Pair count may differ from 5 since atom 2 moved significantly
...
void run_gpu_verlet_vs_cpu_test(int device_id) {
    ...
    gpu_opts.cutoff = cutoff;
    gpu_opts.full = true;
    gpu_opts.sorted = false;
    gpu_opts.algorithm = VesinAutoAlgorithm;
    gpu_opts.return_shifts = true;
    gpu_opts.return_distances = true;
    gpu_opts.return_vectors = true;
    gpu_opts.skin = skin;

    auto gpu_status = vesin_neighbors(
        d_points, n_points, d_box, d_periodic,
